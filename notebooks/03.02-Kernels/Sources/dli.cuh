...
void simulate(dli::temperature_grid_f temp, float *temp_out,
              cudaStream_t stream) {
  symmetry_check(temp, stream);
  dli::simulate(temp, temp_out, stream);
}
...
int main() {
  ...
  for (int compute_step = 0; compute_step < 10; compute_step++) {
    dli::temperature_grid_f temp_in(thrust::raw_pointer_cast(d_prev.data()),
                                    height, width);
