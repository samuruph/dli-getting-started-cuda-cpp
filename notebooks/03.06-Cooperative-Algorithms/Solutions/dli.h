...
void simulate(dli::temperature_grid_f temp_in, float *temp_out) {
  int block_size = 1024;
...
int main() {
  ...
  thrust::universal_vector<float> prev = dli::init(fine_height, fine_width);
  ...
  dli::temperature_grid_f coarse_mdspan(thrust::raw_pointer_cast(coarse_data.data()), coarse_height, coarse_width);

  int write_step = 0;
  int steps = 50000;
  int write_every = steps / 100;
  for (int compute_step = 0; compute_step < steps; compute_step++) {
    dli::temperature_grid_f temp_in(thrust::raw_pointer_cast(prev.data()), fine_height, fine_width);
    dli::simulate(temp_in, thrust::raw_pointer_cast(next.data()));
    prev.swap(next);

    if (compute_step % write_every == 0) {
      dli::temperature_grid_f fine(thrust::raw_pointer_cast(prev.data()), fine_height, fine_width);
      coarse(fine, coarse_mdspan);
      cudaDeviceSynchronize();
      dli::store(write_step++, coarse_height, coarse_width, coarse_data);
    }
  }
}
